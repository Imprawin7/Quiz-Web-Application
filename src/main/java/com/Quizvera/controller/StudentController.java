...
import com.Quizvera.service.AnalyticsService;
import com.Quizvera.service.QuizAttemptService;
import com.Quizvera.service.QuizService;
...
public class StudentController {
    ...
    public String dashboard(Model model, Authentication auth) {
        ...
        List<Quiz> availableQuizzes = quizService.findActiveForStudents();
        List<QuizAttempt> recentAttempts = quizAttemptService.findByStudent(student).stream()
    ...
    @GetMapping("/quizzes")
    public String browseQuizzes(@RequestParam(required = false) String search,
        ...
        List<Quiz> quizzes = quizService.searchActiveForStudents(search, categoryId, difficulty, maxDuration);

        if ("popularity".equals(sortBy)) {
            quizzes = quizzes.stream()
                    .sorted(Comparator.comparingLong(
                            (Quiz q) -> quizAttemptService.countAttemptsForQuiz(q)).reversed())
                    .toList();
        }
        // "newest" is already the natural order returned by the repository (createdAt desc) — no extra sort needed.
    ...
    public String history(Model model, Authentication auth) {
        User student = currentUser(auth);
        List<QuizAttempt> attempts = quizAttemptService.findByStudent(student).stream()
                .filter(a -> a.getStatus() != AttemptStatus.IN_PROGRESS)
                .toList();
